...
The journey from a static Character configuration to a live, interactive Agent involves several distinct phases, each managed by the `AgentRuntime`:

```mermaid
graph LR
    subgraph "Configuration Phase"
        A[Character Definition] --> B[Validation]
        B --> C[Plugin Resolution]
    end
    
    subgraph "Initialization Phase"
        C --> D[Runtime Creation]
