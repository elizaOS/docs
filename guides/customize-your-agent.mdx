...
<Note>
To use a different port, run `elizaos dev --port 8080` (or any port number you prefer).
</Note>
...
Your agent has additional customization options we haven't covered yet. The Character interface includes properties like:
...
For adding large amounts of knowledge to your agent, check out [plugin-knowledge](/plugin-registry/knowledge) which can ingest almost any type of file or media including PDFs, Word docs, markdown, text files, JSON, CSV, XML, and more. It can handle entire document collections, websites, and knowledge bases.
...
Your Shakespeare bot is now live! Invite it to your Discord server and try chatting with it.
