...
<Note>
This guide builds on concepts from [Customize Your Agent](/guides/customize-your-agent). If you're using this as a tutorial, we recommend starting there to follow along.
</Note>

## Introduction

In the [previous guide](/guides/customize-your-agent), we brought Shakespeare to life in Discord. But what if he needs a literary companion? What if we want Shakespeare and Hemingway to converse and share ideas? 

elizaOS makes it simple to run multiple agents in a single project, each with their own unique personality, memory, and ability. Let's go ahead and add Hemingway to our Discord server.
...
Just like in the [previous guide](/guides/customize-your-agent), continue editing the other fields (`topics`, `style`, `messageExamples`, etc.) to match Hemingway as you see fit.
...
<Warning>
Each agent needs its own Discord application and bot token. Follow the Discord setup steps from the [previous guide](/guides/customize-your-agent#create-discord-application) for each agent you create.
</Warning>
...
## Step 3: Launch multi-agent project

### Start your multi-agent server

Now, start the server with your default agent, Shakespeare, from your project root:

```bash Terminal
elizaos start
```

### Check and manage your agents

From a separate terminal tab, let's see which agents are running:

```bash Terminal (Tab 2)
elizaos agent list
```

You should see a table showing your available agents, with Shakespeare as "active" and Hemingway as "inactive".

<Note>
For a complete reference of all agent management commands, see the [CLI Agent Reference](/cli-reference/agent).
</Note>

### Add Hemingway to the conversation

We can see that Hemingway is inactive, so let's start him:

```bash Terminal (Tab 2)
elizaos agent start --path hemingway.json
```

You should see a success message confirming Hemingway has been started. Run `elizaos agent list` again to confirm both agents are now active.
...
<CardGroup cols={2}>
  <Card title="Test Your Agents" icon="flask" href="/guides/test-your-project">
    Ensure your literary duo maintains their unique voices consistently
  </Card>
  <Card title="Deploy Your Agents" icon="rocket" href="/guides/deploy-your-agent">
    Share your Shakespeare vs Hemingway debates with the world
  </Card>
  <Card title="Plugin Development" icon="wand-magic-sparkles" href="/guides/create-and-publish-plugins">
    Build custom plugins to extend your agents' capabilities
  </Card>
  <Card title="Plugin Publishing" icon="upload" href="/guides/plugin-publishing-guide">
