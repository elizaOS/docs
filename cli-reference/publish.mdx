...
<Tabs>
  <Tab title="Overview">
    ...
    | Option | Description |
    | ------ | ----------- |
    | `--npm` | Publish to npm instead of GitHub Packages |
    | `-t, --test` | Run a simulation of the publish process without making changes |
    | `-d, --dry-run` | Generate registry files locally for inspection without publishing |
    | `--skip-registry` | Publish the package but skip submitting to the registry |
  </Tab>
