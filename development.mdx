...
```bash
elizaos add-plugin @elizaos/plugin-twitter
elizaos add-plugin @elizaos/plugin-discord
```

<Warning>
  After installing plugins via CLI, you **must** add them to your `character.json` file to activate them:
</Warning>
...
```bash
elizaos remove-plugin @elizaos/plugin-twitter
```

Remember to also remove it from your `character.json` file.
...
<CardGroup cols={3}>
  ...
  <Card title="Discord Community" icon="discord" href="https://discord.gg/elizaos">
