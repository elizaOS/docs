...
Make sure you have elizaOS installed. If not, follow the [installation guide](/installation) first.

## Create Your First Agent

Let's create an elizaOS project with an AI agent that can chat and take actions.
...
<Steps>
  ...
  <Step title="Start Your Agent">
    ...
    elizaos start
    ```
    Your agent is now running with the default Eliza character!
  </Step>
...
- **`src/character.ts`** - Your agent's personality and configuration
...
Open `src/character.ts` to see your agent's configuration:

```typescript
export const character = {
  name: "Eliza",
  bio: "A friendly AI assistant",
  // ... personality traits, example messages, etc.
}
```

This character file defines your agent's personality, knowledge, and behavior patterns.
...
<CardGroup cols={2}>
  ...
  <Card title="Add Plugins" icon="puzzle-piece" href="/plugins">
    Equip your agent with capabilities like Discord, Twitter, or blockchain interactions
  </Card>
...
<AccordionGroup>
  ...
  <Accordion icon="wrench" title="Other Common Issues">
    ...
    **Need more help?**
    - Check the [installation guide](/installation) for system requirements
    - Visit the [troubleshooting section](/installation#troubleshooting) for detailed solutions
  </Accordion>
</AccordionGroup>